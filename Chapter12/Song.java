...
class Songs {
    ...
    public static void main(String[] args) {
        ...
        List<Song> selectedSongs = songs.getSongs().stream().filter(e->e.getGenre().equals("Rock")).collect(Collectors.toList());
        System.out.println(selectedSongs);
        // Clever filter
        System.out.println("\nChallenge 2");
        List<Song> selectedSongs2 = songs.getSongs().stream().filter(e->e.getGenre().toLowerCase().contains("rock")).collect(Collectors.toList());
        System.out.println(selectedSongs2);
        // Lou’s Challenge #2: List all the genres
        System.out.println("\nChallenge 3");
        List<String> selectedSongs3 = songs.getSongs().stream().map(e->e.getGenre()).distinct().collect(Collectors.toList());
        System.out.println(selectedSongs3);
...
public class Song {
    ...
    public String toString() {
        return String.format("《%s》was written by %s in %d. It has been played for %d and the genre is %s", title, artist, year, timesPlayed, genre);
    }
}
